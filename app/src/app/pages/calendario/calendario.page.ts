		...
		for (let idx = 1; idx <= ultimo_dia.getDate(); idx++) {
			if (idx === 1 && dia_semana !== 0) {
				...
				for (let si = 0; si < dia_semana; si++, n++) {
					this.mes[0][si] = { dia: n, atual: false, selecionado: false, hoje: false, domingo: si === 0 };
				}
			} else if (idx === ultimo_dia.getDate() && ultimo_dia.getDay() !== 6) {
				let n = 0;
				for (let si = ultimo_dia.getDay(); si <= 6; si++, n++) {
					this.mes[semana][si] = { dia: n, atual: false, selecionado: false, hoje: false, domingo: false };
				}
			}
			...
			this.mes[semana][dia_semana] = { dia: idx, atual: true, selecionado: isHoje, hoje: isHoje, domingo: dia_semana === 0 };
